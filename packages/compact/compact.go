...
// SummaryMethod identifies which rung produced a compaction summary.
type SummaryMethod string

const (
	// SummaryModel marks a summary generated by the model.
	SummaryModel SummaryMethod = "model"
	// SummaryFallback marks a deterministic local fallback summary, used when no
	// stream is given or the model produced no usable output.
	SummaryFallback SummaryMethod = "fallback"
)
...
type Result struct {
	...
	Details      any
	// Method records which rung produced Summary: SummaryModel when the model
	// generated non-empty output, otherwise SummaryFallback. It distinguishes
	// model-vs-fallback only for a successful compaction. The third state —
	// compaction failing entirely — is signaled by Run/Harness.Compact returning
	// an error, not by a Method value. An empty Method means unknown (e.g. old
	// data or a caller that does not set it).
...
// always uses the local summary. Result.Method records which rung produced the
// summary (SummaryModel or SummaryFallback).
func Run(ctx context.Context, stream provider.StreamFunc, prep Preparation) (Result, error) {
	...
	return Result{Summary: summary, TokensBefore: estimateTokens(append(prep.Summarize, prep.Keep...)), Messages: messages, Method: method}, nil
}
