...
`subscribe` lets an extension request runtime events by event type. Use `"*"` to request every forwarded event. One-shot `run`, RPC runtimes, and TUI sessions forward subscribed events and apply host actions emitted by event handlers. Event delivery is best-effort and currently one-way; extensions should not block host progress waiting for event acknowledgement.
